<template>
  <div class="settings-container">
    ...
    <el-scrollbar class="scroll" v-if="!firstLoading">
        ...
        <div class="card-grid">
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div><span>Allowed Domains</span></div>
                <div class="forward">
                  <el-button class="opt-button" size="small" type="primary" @click="domainSettingShow = true">
                    <Icon icon="fluent:settings-48-regular" width="18" height="18"/>
                  </el-button>
                </div>
              </div>
          ...
          <div class="settings-card">
            ...
            <div class="card-content">
              <div class="setting-item">
                <div><span>Domain</span></div>
                <div>
                  <el-input v-model="setting.certDomain" placeholder="mail.example.com" />
                </div>
              </div>
              <div class="setting-item">
                <div><span>Email</span></div>
                <div>
                  <el-input v-model="setting.certEmail" placeholder="admin@example.com" />
                </div>
              </div>
              <div class="setting-item">
                <div><span>Auto Renew</span></div>
                <div>
                  <el-switch
                      @change="change"
              ...
              <div class="setting-item">
                ...
                <div class="forward">
                  <span>{{ certRuntime.status || setting.certStatus }}</span>
                  <el-button class="opt-button" size="small" type="primary" @click="loadCertStatus">
              ...
              <div class="setting-item">
                ...
                <div class="r2domain">
                  <span>{{ certRuntime.lastResult || setting.certLastResult }}</span>
                </div>
              </div>
              <div class="setting-item">
                <div><span>Apply / Renew</span></div>
                <div class="forward">
                  <el-button class="opt-button" size="small" type="primary" :loading="certLoading" @click="applyCert">
                    Apply
                  </el-button>
                  <el-button class="opt-button" size="small" type="primary" :loading="certLoading" @click="renewCert">
                    Renew
                  </el-button>
                </div>
              </div>
      ...
      <el-dialog v-model="domainSettingShow" title="Allowed Domains" @closed="resetDomains">
        <div class="prefix-filter">
          <div style="margin-bottom: 10px;">Domains</div>
          <el-input-tag style="margin-bottom: 10px;" v-model="allowedDomains" placeholder="example.com" />
        </div>
        <el-button type="primary" style="width: 100%;" :loading="settingLoading" @click="saveDomains">Save</el-button>
      </el-dialog>
    </el-scrollbar>
  </div>
</template>
...
const domainSettingShow = ref(false)
...
function loadCertStatus() {
  certStatus().then(data => {
    certRuntime.status = data.status
    certRuntime.lastResult = data.lastResult
    certRuntime.lastRunAt = data.lastRunAt
  }).catch((e) => {
    console.error(e)
  })
}
...
function applyCert() {
  ...
  certApply().then(data => {
    ...
    certRuntime.lastRunAt = data.lastRunAt
    ElMessage({
      message: 'Certificate apply finished',
...
function renewCert() {
  ...
  certRenew().then(data => {
    ...
    certRuntime.lastRunAt = data.lastRunAt
    ElMessage({
      message: 'Certificate renew finished',
      type: data.status === 'ok' ? 'success' : 'warning',
      plain: true,
    })
  }).finally(() => {
...
function resetDomains() {
  allowedDomains.value = [...(setting.value.allowedDomains || [])]
}
...
function saveDomains() {
  const form = {}
  form.allowedDomains = allowedDomains.value.map(item => (item || '').trim().toLowerCase()).filter(Boolean)
  editSetting(form, true)
}
